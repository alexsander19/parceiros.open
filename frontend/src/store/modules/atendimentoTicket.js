...
const checkTicketFilter = (ticket) => {
  ...
  // se configuração para equipe ativa: verificar se já é um ticket da equipe do usuário
  if (DirectTicketsToWallets() && (ticket?.contact?.wallets?.length || 0) > 0) {
    const idx = ticket?.contact?.wallets.findIndex(w => w.id == userId)
    if (idx !== -1) {
      console.log('Ticket da equipe do usuário')
      return true
    }
    console.log('DirectTicketsToWallets: Ticket não pertence à equipe do usuário', ticket)
    return false
  }
